	...
	* ya que el arbol se encuentra ordenado por isbn}
	...
	procedure recorrerArbolIncisoI (a : arbol1; valor1,valor2 : integer; var totalPrestamos : integer); 
	begin
		if (a <>nil) then 
			begin
				recorrerArbolIncisoI(a^.HI,valor1,valor2,totalPrestamos); 
				{ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -}
				if (a^.dato.isbn >= valor1) and (a^.dato.isbn<= valor2) then 
					begin
						totalPrestamos:= totalPrestamos+1; 
					end; 
			    { - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -}
				recorrerArbolIncisoI(a^.HD,valor1,valor2,totalPrestamos); 
			end; 
	end; 
	
	
	function recorrerArbolIncisoIv2 (a : arbol1; valor1,valor2: integer) : integer; 
	begin
		if (a = nil) then 
			recorrerArbolIncisoIv2:= 0
		else
			begin
				recorrerArbolIncisoIv2:= recorrerArbolIncisoIv2(a^.HI,valor1,valor2) + recorrerArbolIncisoIv2 (a^.HD,valor1,valor2); 
					if (a^.dato.isbn >= valor1) and (a^.dato.isbn <= valor2) then 
						recorrerArbolIncisoIv2:= recorrerArbolIncisoiv2 +1; 
			end;
	end; 

	procedure moduloI (a : arbol1); 
	var
		valor1,valor2,totalPrestamos : integer;
	begin
		totalPrestamos:= 0;
		writeln ('ESTAS EN EL INCISO I ');
		write ('Ingrese el valor 1: '); readln (valor1); 
		write ('Ingrese el valor 2: '); readln (valor2);
		recorrerArbolIncisoI (a,valor1,valor2,totalPrestamos); 	
		writeln ('En el procedure se encontraron ',totalPrestamos, ' prestamos entre medio de esos valores (inclusive los limites)'); 
			{prueba de que se puede hacer con procedure y funcion}
		writeln ('En la funcion se encontraron ',recorrerArbolIncisoIv2(a,valor1,valor2), ' prestamos entre medio de esos valores (inclusive los limites)'); 
	end; 
	{ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  }	
		{ 	j. Un módulo recursivo que reciba la estructura generada en ii. y dos valores de ISBN. El
	módulo debe retornar la cantidad total de préstamos realizados a los ISBN
	comprendidos entre los dos valores recibidos (incluidos).} 
		function recorrerLista (pI : listaPrestamos; valor1,valor2 : integer) : integer; 
		begin
			if (pI = nil) then 
				recorrerLista:= 0
			else
				begin
						if  ( (pI^.dato.isbn>= valor1) and (pI^.dato.isbn <= valor2) )  then 
							recorrerLista:= recorrerLista(pI^.sig,valor1,valor2) + 1 
						else
							recorrerLista:= recorrerLista(pI^.sig,valor1,valor2) ; 
				end;
		end; 
		
		function recorrerNodo (a2 : arbol2 ; valor1,valor2 : integer) : integer; 
		begin
			if (a2 <> nil) then 
				begin
					recorrerNodo:= recorrerLista(a2^.dato,valor1,valor2) + recorrerNodo(a2^.HI,valor1,valor2) + recorrerNodo(a2^.HD,valor1,valor2); 
				end
			else
				recorrerNodo:= 0; 
		...
		procedure moduloJ (a2 : arbol2); 
		var
			valor1,valor2 : integer;
		begin
			writeln ('ESTAS EN EL INCISO J ');
			write ('Ingrese el valor 1: '); 
			readln (valor1); 
			write ('Ingrese el valor 2: '); 
			readln (valor2); 
			writeln ('La cantidad de prestamos entre los valores son ',recorrerNodo(a2,valor1,valor2)); 
		end; 
