	...
	* ya que el arbol se encuentra ordenado por isbn}
	...
	begin
		if (a2 <>nil) then 
			begin
			//	contarEnNodos:= contarEnLista(a2^.dato,nroSocio); 
	...
	ordenada ISBN, donde cada ISBN aparezca una vez junto a la cantidad total de veces
	que se prestó. }
	procedure inicializarListaF ( var l : prestamosISBN); 
	begin
		l:= nil; 
	end; 
	

	procedure agregarAdelante (var pI : prestamosISBN ; dato : rIncisoF); 
	var
		nue : prestamosISBN;
	begin
		new (nue); 
	...
	procedure actualizarPrestamo (aux : prestamosISBN); 
	begin
	  aux^.dato.cantPrestamos:= aux^.dato.cantPrestamos + 1; 
	end;

	procedure asignarDatos (var aux : rIncisoF; isbn : integer); 
	begin
		aux.isbn:= isbn; 
		aux.cantPrestamos:= 1; 
	end;

	procedure cargarListaF (var pI : prestamosISBN; a1 : arbol1);
	begin
		if (a1<> nil) then 
			begin
...
procedure recorrerArbol1(a1: arbol1; var pI: prestamosISBN);
begin
    if (a1 <> nil) then
	  begin
		recorrerArbol1(a1^.HD, pI);
		cargarListaF (pI,a1); 
        recorrerArbol1(a1^.HI, pI);
      end;
end;

	procedure moduloF (a1 : arbol1 ; var pI : prestamosISBN); 
	begin
		inicializarListaF(pI); 
		recorrerArbol1(a1,pI); 
	end; 
